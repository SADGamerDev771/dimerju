...
export default function FloatingElement({
  ...
  const startFloatingAnimation = async () => {
    // Generate random destination points across the entire page
    ...
    while (isVisible && !isDragging) {
      const destination = generateRandomDestination()

      // Calculate duration based on distance for smoother movement
      const distance = Math.sqrt(Math.pow(destination.x - position.x, 2) + Math.pow(destination.y - position.y, 2))
      const duration = distance * 0.1 // Adjust this multiplier for speed
    ...
    controls
      ...
      .then(() => {
        ...
        setTimeout(() => {
          ...
          // Make visible again with animation
          setIsVisible(true)
          controls.set({ scale: 0, opacity: 0, rotate: 0 })
          controls.start({
            scale: 1,
            opacity: 1,
            transition: { duration: 0.5 },
